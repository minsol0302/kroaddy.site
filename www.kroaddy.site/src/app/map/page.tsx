...
export default function KakaoMapPage() {
    ...
    useEffect(() => {
        ...
        const initMap = () => {
            ...
            if (!window.kakao.maps) {
                ...
                console.error("가능한 원인: 스크립트가 완전히 로드되지 않았거나 API 키가 유효하지 않습니다.");
                return;
            }
            ...
            try {
                ...
                console.log("✅ 카카오 지도가 성공적으로 로드되었습니다.", map);
            } catch (error) {
        ...
        if (window.kakao && window.kakao.maps) {
            ...
            const handleKakaoMapLoad = () => {
                ...
                // 약간의 지연을 두고 초기화 (스크립트가 완전히 준비될 때까지)
                setTimeout(() => {
                    initMap();
                }, 200);
            };
    ...
    return (
        <div className="min-h-screen p-8">
            ...
            {/* 카카오 지도 스크립트 불러오기 - 가이드에 따라 autoload 파라미터 제거 */}
            {KAKAO_MAP_API_KEY && (
                <Script
                    ...
                    onLoad={() => {
                        ...
                        // 약간의 지연 후 지도 초기화 (스크립트가 완전히 로드될 때까지 대기)
                        setTimeout(() => {
